...
from .sae import SAE_CLASSES, Sae
...
class SaeTrainer:
    def __init__(
        ...
        self.model = model
        sae_class = SAE_CLASSES[cfg.arch_type]
        self.saes = {
            hook: sae_class(input_widths[hook], cfg.sae, device)
    ...
    def fit(self):
        ...
        for batch in dl:
            ...
            cur_loss /= len(output_dict) * self.cfg.micro_acc_steps
            log_loss = cur_loss * 0.01 + log_loss * 0.99
            pbar.set_postfix({"loss": "%.3f"%log_loss})
