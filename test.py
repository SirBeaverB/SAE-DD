...
with torch.inference_mode(): # no gradient
    for pn, sentences in words.items():
...
positive_indices_range = range(0, 20)  # 正例的索引范围
negative_indices_range = range(20, 40)  # 负例的索引范围
...
for key, count in most_common:
    ...
    # 初始化y_true和y_pred
    y_true_positive = [0] * 40  # 初始化长度为40的列表
    y_pred_positive = [0] * 40  # 初始化长度为40的列表
    y_true_negative = [0] * 40  # 初始化长度为40的列表
    y_pred_negative = [0] * 40  # 初始化长度为40的列表

    # 为正例和负例分别构建y_true和y_pred
    for idx in range(40):  # 迭代所有样本
        # 对于正例
...
for i, result in enumerate(top_diff[:5]):
