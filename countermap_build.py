...
def invert_embeddings(input_pattern, output_file):
    """
    读取符合 input_pattern (如 'embeddings_with_tokens_part_*.json') 的所有 JSON 文件，
    将其“反转”后写入 output_file。
    ...
    # 找到所有符合模式的文件，排序便于可控的处理顺序
    input_files = sorted(glob.glob(input_pattern))
    print(f"发现待处理文件: {input_files}")
...
if __name__ == "__main__":
    ...
    # 再将结果输出到 'inverted_embeddings.json'
    invert_embeddings("embeddings_with_tokens_part_1.json", "inverted_embeddings.json")